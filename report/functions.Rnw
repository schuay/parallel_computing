...
makeMinSpeedupTables <- function(file) {
	results <- read.table(file, sep=",")

	time_mean <- aggregate(results[,4], by=list(results[,1],results[,2],results[,3]), FUN=min)

	unused <- makeSpeedupTables(time_mean, "Min")
}
