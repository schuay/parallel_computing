...
These steps are repeated for each iteration. Upon conclusion, the main matrix is
reformed after gathering all submatrices.
...
The value of $R$ is not important as long as $R >= p$ and the elements $x \in xs$
